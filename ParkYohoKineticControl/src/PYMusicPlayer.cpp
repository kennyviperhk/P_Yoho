...
float PYMusicPlayer::pause(bool t) {
    mp3.setPaused(t);
    //mp3.stop();
...
long PYMusicPlayer::getDuration() {
    computeSoundDuration();
    return duration;
}

float PYMusicPlayer::getCurrentPos() {
    float playheadPos = 0;
    if (mp3.getIsPlaying()) {
    }
    return mp3.getPosition();
...
void PYMusicPlayer::setPos(float pos) {
    mp3.setPosition(pos);
}

void PYMusicPlayer::setVol(float vol) {
    mp3.setVolume(vol);
}


void PYMusicPlayer::computeSoundDuration() {
...
void PYMusicPlayer::setup() {
	...
	for (int i = 0; i < 20; i++) {
		Timeline timeline;
		timeline.setup();
		ofAddListener(timeline.keyframeSelectedEvent, this, &PYMusicPlayer::keyFrameSelected);
		ofAddListener(timeline.keyframeDeselectedEvent, this, &PYMusicPlayer::keyFrameDeselected);
	...
	//-----------------------     GUI      -------------------
	playButton.addListener(this, &PYMusicPlayer::playButtonPressed);
	nextButton.addListener(this, &PYMusicPlayer::nextButtonPressed);
	addKeyButton.addListener(this, &PYMusicPlayer::addKeyButtonPressed);
	removeKeyButton.addListener(this, &PYMusicPlayer::removeKeyButtonPressed);
	selectKeyButton.addListener(this, &PYMusicPlayer::selectKeyButtonPressed);
	pauseButton.addListener(this, &PYMusicPlayer::pauseButtonPressed);
	volume.addListener(this, &PYMusicPlayer::volumeChanged);
	graphSlide.addListener(this, &PYMusicPlayer::graphScrollChanged);
	keyframeSlider.addListener(this, &PYMusicPlayer::keyframeSliderChanged);
	saveButton.addListener(this, &PYMusicPlayer::saveButtonPressed);
	loadButton.addListener(this, &PYMusicPlayer::loadButtonPressed);
	...
	gui.loadFromFile("PYMusicPlayerSettings.xml");
     */
}

//--------------------------------------------------------------
void PYMusicPlayer::update() {
...
vector<float> PYMusicPlayer::getTimelineTweenValues() {
	float graphWidth = duration * 1.0f * graphScale;
	float playheadPos = 0.0f;
	if (mp3.getIsPlaying()) {
		playheadPos = graphWidth*mp3.getPosition();
...
void PYMusicPlayer::keyframeSliderChanged(float &val) {
	if (selectedKeyframe->x != nullKeyframe.x) {
		selectedKeyframe->val = val;
	}
}

void PYMusicPlayer::keyFrameSelected(Keyframe &kf) {
...
void PYMusicPlayer::keyFrameDeselected(int &i) {
	selectedKeyframe = &nullKeyframe;
...
void PYMusicPlayer::addKeyButtonPressed() {
	for (int i = 0; i < 20; i++) {
		timelines[i].addKeyframeOnClick();
	}
}

void PYMusicPlayer::removeKeyButtonPressed() {
	for (int i = 0; i < 20; i++) {
		timelines[i].removeKeyframeOnClick();
	}
}

void PYMusicPlayer::selectKeyButtonPressed() {
	for (int i = 0; i < 20; i++) {
...
/*
//--------------------------------------------------------------
void PYMusicPlayer::playButtonPressed() {
	computeSoundDuration();
	resetGraph();
	reloadTimelineFromSave();
	mp3.play();
...
//--------------------------------------------------------------
void PYMusicPlayer::nextButtonPressed() {
...
void PYMusicPlayer::pauseButtonPressed() {
	if (mp3.isPlaying()) {
...
void PYMusicPlayer::mousePressed(int x, int y, int button) {
...
void PYMusicPlayer::saveButtonPressed() {
...
void PYMusicPlayer::loadButtonPressed() {
	reloadTimelineFromSave();
}

void PYMusicPlayer::reloadTimelineFromSave() {
...
//--------------------------------------------------------------
void PYMusicPlayer::volumeChanged(float &setVolume) {
	mp3.setVolume(setVolume);
}

void PYMusicPlayer::graphScrollChanged(float &setScroll) {
	graphScrollX = setScroll;
}


void PYMusicPlayer::resetGraph() {
	fftHistory.clear();
	timeHistory.clear();
	lastMaxSoundTime = 0;
	lastPauseSoundTime = 0;
