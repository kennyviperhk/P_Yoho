...
void ofApp::guiDraw(){
    ...
    if(drawKineticVisualizationFbo){
        ofSetColor(255);
        ofPushMatrix();
            kineticVisualizationFbo.draw(0,0,ofGetWidth()*0.5,ofGetHeight()*0.5);
        ofPopMatrix();
    }
...
*/
    if(cb == 0){
        ...
        output_pts[1] = false;
        output_pts[3] = true;
    }else if(cb == 6){//TFTF , FTTF, TFTF, FTFT
        output_pts[1] = true;
        output_pts[3] = false;
...
void ofApp::musicAndTimelineSetup(){
    drawMusicPlayer = false;
    musicPlayer.setup();
