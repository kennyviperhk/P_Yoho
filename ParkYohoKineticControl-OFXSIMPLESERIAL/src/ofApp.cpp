    ...
    //================== Debug Mode ==================
        ...
        for(int i=0; i < NUM_OF_CABLES; i++){
            ...
            if(isArduinoConnectedBySerial[i]){
                ...
                ss2 << "Connected Devices (" << i << ") : " << arduinoPortName[i] << "receivedMsg: " << prevReceivedString[i] << endl;
            }else{
                ofSetColor(0,0,0);
...
vector<bool> ofApp::serialSetup(){ //int give the connection status of each cables
    ...
    vector<ofSerialDeviceInfo> devicesInfo = s.getDeviceList();
    ofLogNotice("ofApp::setup") << "Connected Devices: ";
    for (std::size_t i = 0; i < devicesInfo.size(); ++i)
    {
        ofLogNotice("ofApp::getDeviceID") << "\t" << devicesInfo[i].getDeviceID();
        ofLogNotice("ofApp::getDevicePath") << "\t" << devicesInfo[i].getDevicePath();
        ofLogNotice("ofApp::getDeviceName") << "\t" << devicesInfo[i].getDeviceName();
    }
    int a=0;
    if (!devicesInfo.empty())
    {
        ...
        for (std::size_t i = 0; i < devicesInfo.size(); ++i)
        {
            string portDesc = devicesInfo[i].getDevicePath();
          //  ofLog() << "devicesInfo[i].getDevicePath() : " << devicesInfo[i].getDevicePath();
            
            if(portDesc.find("tty.usb") != std::string::npos || portDesc.find("Arduino") != std::string::npos )
            {
                
                ofxSimpleSerial device;
                 bool success = device.setup(i, BAUD);
                ...
                if(success)
                {
                    ...
                    arduinoPortName.push_back(devicesInfo[i].getDevicePath());
                    arduino[a].startContinuousRead(false);
                    ofAddListener(arduino[a].NEW_MESSAGE,this,&ofApp::onNewMessage);
                    ofLogNotice("ofApp::setup") << "Successfully setup " << devicesInfo[i].getDevicePath();
                    a++;
                }
                else
                {
                    ofLogNotice("ofApp::setup") << "Unable to setup " << devicesInfo[i].getDevicePath();
                }
            }
        }
    }
...
void ofApp::serialWrite(int arduinoID, string sw){
    ...
    else if (isArduinoConnected[arduinoID]==TRUE && arduinoID >= 0)
    {
        ...
        arduino[arduinoID].writeString(sw);
        // The serial device can throw exeptions.
       /* try
        {
            std::string text = sw;
